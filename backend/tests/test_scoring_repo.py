...
def test_fetch_contract_value_returns_the_accounts_value():
    client = FakeSupabaseClient(
        {"account": [{"id": "a1", "contract_value_monthly": 5000}]}
    )
...
def test_fetch_accounts_with_contract_value_maps_every_account():
    ...
    assert fetch_accounts_with_contract_value(client, TEST_AGENCY_ID) == {
        "a1": (5000.0, "Acme"),
        "a2": (12000.0, "Beta"),
    }
...
def test_upsert_alert_does_not_duplicate_when_severity_is_unchanged():
    ...
    assert len(client._tables["alert"]) == 1
    assert (
        client._tables["alert"][0]["triggered_at"] == "2026-01-01T00:00:00"
    )  # untouched
...
def test_upsert_alert_escalates_existing_alert_in_place():
    ...
    alert_id = upsert_alert(
        client, "a1", ["invoice_days_late", "meetings_cancelled"], "high"
    )
    rows = client._tables["alert"]
    ...
    assert (
        rows[0]["triggered_at"] == "2026-01-01T00:00:00"
    )  # preserved: still the original trigger time
    assert alert_id == "existing"
...
def test_upsert_alert_refreshes_signals_fired_even_without_severity_escalation():
    ...
    assert (
        rows[0]["triggered_at"] == "2026-01-01T00:00:00"
    )  # original trigger time preserved
