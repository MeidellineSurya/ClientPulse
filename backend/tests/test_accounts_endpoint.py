    ...
    fake_client = FakeSupabaseClient(
        {
            ...
            "health_score": [
                ...
                {"account_id": "acc-1", "composite_score": 72.5, "trend_slope": 3.2, "computed_at": "2026-02-01T00:00:00"},
            ],
        }
    )
...
def test_list_accounts_account_never_scored_has_null_score_fields():
    fake_client = FakeSupabaseClient(
        {"account": [{"id": "acc-1", "name": "Acme", "contract_value_monthly": 5000}], "health_score": []}
    )
    client = _override_client(fake_client)
    try:
        response = client.get("/accounts")
    ...
    fake_client = FakeSupabaseClient(
        {
            ...
            "health_score": [
                {"account_id": "acc-1", "composite_score": 90.0, "trend_slope": 5.0, "computed_at": "2026-02-01T00:00:00"}
            ],
...
def test_get_account_detail_404_when_account_missing():
    ...
    client = _override_client(fake_client)
    try:
        response = client.get("/accounts/ghost")
    finally:
        app.dependency_overrides.pop(require_supabase_client, None)
...
def test_get_account_signals_returns_history():
    fake_client = FakeSupabaseClient(
        {
            "account": [{"id": "acc-1", "name": "Acme", "contract_value_monthly": 10000}],
            "signal_snapshot": [_snapshot("acc-1", "2026-01-01"), _snapshot("acc-1", "2026-01-08", "2026-01-14")],
        }
    )
    client = _override_client(fake_client)
    try:
        response = client.get("/accounts/acc-1/signals")
    finally:
        app.dependency_overrides.pop(require_supabase_client, None)
...
def test_get_account_signals_404_when_account_missing():
    ...
    client = _override_client(fake_client)
    try:
        response = client.get("/accounts/ghost/signals")
    finally:
        app.dependency_overrides.pop(require_supabase_client, None)
