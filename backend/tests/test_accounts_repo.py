...
def test_fetch_all_accounts_returns_every_row():
    client = FakeSupabaseClient({"account": [{"id": "a1", "name": "Acme"}, {"id": "a2", "name": "Beta"}]})
    assert [a["id"] for a in fetch_all_accounts(client)] == ["a1", "a2"]


def test_fetch_account_returns_the_matching_row():
    client = FakeSupabaseClient({"account": [{"id": "a1", "name": "Acme"}]})
    assert fetch_account(client, "a1")["name"] == "Acme"


def test_fetch_account_returns_none_when_missing():
    client = FakeSupabaseClient({"account": []})
    assert fetch_account(client, "ghost") is None
...
def test_fetch_latest_health_scores_picks_the_most_recent_row_per_account():
    ...
    latest = fetch_latest_health_scores(client)
...
def test_fetch_full_signal_history_sorted_oldest_first():
    client = FakeSupabaseClient(
        {
            "signal_snapshot": [
                {"account_id": "a1", "period_start": "2026-02-01", "period_end": "2026-02-07",
                 "avg_response_time_hours": 5, "meetings_scheduled": 2, "meetings_cancelled": 0,
                 "invoice_days_late": 0, "email_thread_count": 10},
                {"account_id": "a1", "period_start": "2026-01-01", "period_end": "2026-01-07",
                 "avg_response_time_hours": 3, "meetings_scheduled": 3, "meetings_cancelled": 0,
                 "invoice_days_late": 0, "email_thread_count": 8},
            ]
        }
    )
